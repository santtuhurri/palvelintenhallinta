...
```
	/usr/games/tulokset.txt:
	  file.managed:
	    ...
	    - mode: 777
```
Nyt kaikilla käyttäjillä on oikeudet muokata tiedostoa ja testasin tätä ajamalla tilan uudestaan sekä onnistuneesti muokkaamalla tiedostoa. (Update 28.4. HUOM! tiedostolle ei tulisi ikinä antaa 777 oikeuksia, koska tämä on suuri tietoturvariski. Tämä antaa kaikille käyttäjille mahdollisuuden muokata kyseistä tiedostoa. 755 antaa riittävät oikeudet ja mahdollistaa tiedoston lukemisen ja suorittamisen. Tässä harjoituksessa halusin kuitenkin mahdollistaa tiedoston muokkaamisen kaikille käyttäjille. )
